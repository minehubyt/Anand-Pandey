            ...
            return (
              <div key={index} className={`flex-none w-[75vw] px-2 md:px-4 transition-all duration-[900ms] ${isActive ? 'scale-100 opacity-100' : 'scale-[0.92] opacity-50'}`}>
                <div 
                  ...
                  onClick={() => isActive && onInsightClick(item.id, item.title)}
                >
                  ...
                  <div className={`relative z-30 h-full flex flex-col justify-center px-8 md:px-20 max-w-4xl text-white transition-all duration-[1s] ${isActive ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0'}`}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); onInsightClick(item.id, item.title); }} className="text-[11px] md:text-[13px] font-bold tracking-[0.3em] uppercase border-b-2 border-white pb-1.5 inline-block hover:text-[#CC1414] hover:border-[#CC1414] transition-all self-start">READ MORE</button>
                  </div>
                </div>
              </div>
            );
